...
class Controller(object):
  ...
  def _sample_episodes(self, sess, start_step = 0, greedy=False):
    ...
    # FIXME Why is this wrapping the result in an array,
    ...
    while not self.env.all_done():
      ...
      next_obs, reward, next_dones, info = self.env.step(env_actions)
      # FIXME
      #next_obs = next_obs[0]
      ...
      if ep_writer: 
          for i in range(len(next_obs[0])):
              ...
              for _info in info_vals:
                  if isinstance(_info, list) or isinstance(_info, np.ndarray):
                      for k in _info:
                          vals.append(k)
                  else:
                      vals.append(_info)
    ...
    # If we were logging the episode close it
    if ep_file: ep_file.close()
  ...
  def sample_episodes(self, sess, greedy=False):
    ...
    while total_steps < self.max_step * len(self.env):
      (initial_state,
       ...
       pads) = self._sample_episodes(sess, start_step = total_steps, greedy=greedy)
